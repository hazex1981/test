...
fields = %w(食品 タバコ 試料 食料 医薬品 化粧品 医療用品 化学工業品 石油 石炭
            ...
            インターネット コンサルティング リサーチ 人材関連 総合商社 専門商社 小売 飲食娯楽店 旅行 教育
            学習支援 医療 福祉 広告 出版 エンターテイメント 調査 官公庁 公共機関 B２B
            B２C 小売  消費者サービス シンクタンク 会計事務所 通信キャリア 店舗・FC開発 消費者サービス 医療
            福祉  公共サービス 建設 不動産 金融 保険 運輸 10名以下 11～100名 101～1000名
            1001～5000名  5001名以上)
#ループでまわす。
...
#SKILLというジャンルに項目をいれる。
skills = %w(
            成長戦略 M&A・アライアンス対策 組織改革 オペレーション 会計・税務 マーケティング システム 商品企画・開発 ユーザーリレーション ERP SCM
            SFA・データマイニング アプリケーションサーバ ストレージ管理 統合運用管理  グループウェア RDBMS   UXデザイン  UIデザイン サービスコンセプト設計 ユーザーインタビュー
            SEO対策 PMP CSPO ソフトウェア開発 ビックデータ 事業立上げ支援)
...
jobs = %w(
          ...
          パッケージソフト・ミドルウェア開発   通信ネットワーク設計・構築   運用  監視  テクニカルサポート   保守  社内情報システム    MIS SEOコンサルティング)
